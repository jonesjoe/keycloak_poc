...
public class DemoUserStorageProvider implements UserStorageProvider,
  ...
  public List<UserModel> getUsers(RealmModel realm) {
    log.infov("list users: clinetssssss={0}", realm.getClients());
    List<UserModel> models= repository.getAllUsers().stream()
  ...
  public List<UserModel> searchForUser(Map<String, String> params, RealmModel realm) {
    List<UserModel> models= repository.getAllUsers().stream()
      .map(user -> new UserAdapter(session, realm, model, user))
      .collect(Collectors.toList());
      log.infov("searchForUser 195 list users: models={0}", models);
      return models;
  }
